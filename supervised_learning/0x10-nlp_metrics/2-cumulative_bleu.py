...
def cumulative_bleu(references, sentence, n):
    """
    calculates cumulative n-gram BLEU score for sentence
    """
    value = 1 / n
    W = [value for x in range(n)]
    Pn = [modified_precision(
        references, sentence, ngram) for ngram, _ in enumerate(W, start=1)]
    ...
    if len(sentence) > r:
        BP = 1
    score = np.sum([(
        wn * np.log(Pn[i])) if Pn[i] != 0 else 0 for i, wn in enumerate(W)])
